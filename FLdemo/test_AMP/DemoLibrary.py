...
class FLClient:
    def __init__(self, model, train_data, device):
        ...
        self.device = device
        # 梯度缩放器（仅在GPU且启用AMP时生效）
        self.scaler = torch.cuda.amp.GradScaler(
            enabled=FLConfig.AMP_ENABLED and str(device) == FLConfig.AMP_DEVICE
        )
        ...
        for _ in range(FLConfig.LOCAL_EPOCHS):
            for images, labels in self.train_loader:
                ...
                with torch.cuda.amp.autocast(
                        ...
                        dtype=torch.float16 if self.scaler.is_enabled() else torch.float32
                ):
